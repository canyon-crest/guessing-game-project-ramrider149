    ...
    if (!hintsUsed.includes("comparison") && level >= 10) {
        ...
        availableHints.push({
            ...
            text: answer > comparePoint 
                ? `The number is GREATER than ${comparePoint} `
                : `The number is LESS than or equal to ${comparePoint} `
        });
    }
...
function getTemperatureHint(userGuess) {
    ...
    if (percentDiff <= 5) {
        ...
        return " HOT! Getting very warm!";
    } else if (percentDiff <= 30) {
        return " Warm! You're in the right area!";
    } else if (percentDiff <= 50) {
        return " Cool. You're a bit far.";
    } else {
        return "FREEZING COLD! Way off!";
    }
}
...
function updateStreak(won) {
    if (won) {
        ...
        document.querySelector('.streak-section').classList.add('celebrating');
        setTimeout(() => {
            document.querySelector('.streak-section').classList.remove('celebrating');
        }, 1500);
    } else {
        currentStreak = 0;
        document.getElementById('currentStreak').textContent = currentStreak;
